...
const Bubble: FC<BubbleProps> = ({ horizontalPosition, answer }) => {
  ...
  return (
    <div
      className={`w-56 h-56 rounded-lg opacity-50 shadow-xl overflow-hidden absolute transition-all duration-50000 ${
        animating ? "-top-full" : "top-full"
      }`}
    ...
    <div className="w-full h-full overflow-hidden absolute top-0 left-0 z-0">
      {playerAnswers.map((answer) => (
        <Bubble
          key={answer.id}
          horizontalPosition={
            horizontalPositions[answer.id % horizontalPositions.length]
          }
          answer={answer}
        />
      ))}
