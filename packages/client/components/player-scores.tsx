    ...
    {scores.map((playerScore, index) => {
      ...
      return <div className="my-5">
        ...
        <div key={playerScore.player.username} className="relative mt-2 space px-2 p-1 md:p-2 md:px-4 rounded-lg overflow-hidden bg-gray-800">
          ...
          >
            {playerScore.added &&
              <div className="h-full bg-pink-400 w-0 absolute right-0 text-pink-800 items-center justify-center flex" style={{ width: `${Math.round(playerScore.added * 100 / playerScore.score)}%` }}>
                +{playerScore.added}
              </div>
            }
          </div>
