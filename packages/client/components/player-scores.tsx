  ...
  return <div>
    {scores.map((playerScore, index) => {
      ...
      return <div className="my-5">
        <h3 className="font-bold text-blue-100 flex items-baseline">
          {index + 1}: {playerScore.player.username} ({playerScore.score})
          <small className="ml-2">{answeredInText}</small>
        </h3>
        <div key={playerScore.player.username} className="relative mt-2 space px-2 p-1 md:p-2 md:px-4 rounded-lg overflow-hidden bg-gray-800">
          <div
          ...
          >
            {playerScore.added &&
              <div className="h-full bg-blue-300 w-0 absolute right-0 text-blue-800 items-center justify-center flex" style={{ width: `${Math.round(playerScore.added * 100 / playerScore.score)}%` }}>
                +{playerScore.added}
              </div>
            }
          </div>
